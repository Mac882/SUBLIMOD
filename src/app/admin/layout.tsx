...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`${mobile ? "w-[min(86vw,300px)]" : collapsed ? "w-[82px] shrink-0" : "w-[286px] shrink-0"} flex h-full flex-col border-r border-white/10 bg-[#191919] shadow-2xl transition-[width] duration-300`}>
  ...
  return <>
    <style>{`
      .admin-legacy-content { min-height: 100%; width: 100%; overflow-y: auto; }
      .admin-legacy-content > div { min-height: 100%; width: 100%; display: block !important; }
    ...
    `}</style>
    <div className="pointer-events-none fixed inset-0 z-[1000] flex bg-[#121212]">
      <div className="pointer-events-auto hidden h-full md:block"><Sidebar /></div>
      <div className="pointer-events-auto flex min-w-0 flex-1 flex-col">
        {!dashboardOpen && <div data-admin-legacy="visible" className="admin-legacy-content">{children}</div>}
        {dashboardOpen && <main className="admin-dashboard-main h-full min-w-0 flex-1 overflow-y-auto bg-[#121212] text-gray-200">
