...
function TechnologyManager() {
  ...
  return <main className="min-h-screen bg-[#111] p-5 text-white md:p-10"><div className="mx-auto max-w-7xl">
    ...
    {activeSection === "products" && <section className="grid gap-8 lg:grid-cols-[1fr_1.3fr]">
      ...
      <div className="space-y-3">{products.map((product) => { const isAvailable = product.available !== false; return <article key={product.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs uppercase text-primary">{categoryMap.get(product.categoryId)?.name || "Categoría eliminada"}</p><h3 className="mt-1 font-black uppercase">{product.name}</h3><p className="mt-1 text-xs text-gray-500">{product.brand} · {product.model} · {product.sku}</p><p className="mt-2 text-sm font-bold">{product.currency === "USD" ? "$" : "C$"} {Number(product.price).toLocaleString()}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => toggleProductAvailability(product)} className={`rounded-lg px-3 py-2 text-xs font-bold ${isAvailable ? "bg-green-500/15 text-green-300" : "bg-red-500/15 text-red-300"}`}>{isAvailable ? "Disponible" : "No disponible"}</button><button onClick={() => { setEditingProduct(product.id || null); setProductForm({ ...emptyTechnologyProduct, ...product, available: product.available !== false }); }} className="rounded-lg bg-white/10 px-3 py-2 text-xs">Editar</button><button onClick={async () => { if (product.id) await deleteTechnologyProduct(product.id); }} className="rounded-lg bg-red-500/10 px-3 py-2 text-xs text-red-300">Eliminar</button></div></div></article>; })}{products.length === 0 && <p className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm text-gray-500">No hay productos tecnológicos. El catálogo está limpio.</p>}</div>
    </section>}
  </div></main>;
}

export default function AdminTechnologyPage() { return <AuthGuard><TechnologyManager /></AuthGuard>; }
