...
import { 
  ...
  MessageSquare, Save, X, CheckCircle, Tag, Hash, Info, Search, Filter, Check, XCircle, AlertTriangle
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-gray-200 flex flex-col md:flex-row font-sans">
      ...
      <aside className="hidden md:flex w-72 bg-[#1E1E1E] border-r border-white/5 flex-col sticky top-0 h-screen shrink-0">
        ...
        <NavLinks />
        <div className="p-8 border-t border-white/5 text-[10px] text-gray-600 font-bold uppercase tracking-widest">v1.0.7 SECURITY</div>
      </aside>
        ...
        {activeTab === "configuracion" && (
          <section className="max-w-3xl space-y-10 animate-in fade-in pb-16">
            ...
            <div className="bg-[#1E1E1E] p-10 rounded-[2.5rem] border border-white/5 space-y-8 shadow-2xl relative">
              ...
              {/* CONTENIDO NOSOTROS Y MULTIMEDIA */}
                ...
                <div className="space-y-4">
                  ...
                  {config.aboutImageUrl && (
                    <div className="relative w-40 h-40 rounded-2xl overflow-hidden border border-white/10 mb-3 group">
                      ...
                      <button 
                        type="button"
                        onClick={() => setConfig({...config, aboutImageUrl: ""})}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            reader.onloadend = () => {
                              setConfig({ ...config, aboutImageUrl: reader.result as string });
                            };
                    ...
                    <input 
                      ...
                      className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 outline-none text-white focus:border-primary text-xs font-bold" 
        ...
        {selectedProductForPreview && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[250] flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-[#1A1A1A] w-full max-w-5xl my-auto rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden animate-in zoom-in duration-300 relative flex flex-col md:flex-row">
              <button onClick={() => setSelectedProductForPreview(null)} className="absolute top-6 right-6 p-3 bg-white/5 rounded-full text-gray-400 hover:text-white z-50">
                <X size={24}/>
              </button>
              ...
              <div className="bg-[#121212] p-10 flex flex-col items-center justify-center border-r border-white/5 md:w-1/2 min-h-[300px]">
                {previewImagesList.length > 0 && (
                  <img src={previewImagesList[activePreviewImage]} className="w-full h-auto max-h-[50vh] object-contain rounded-3xl drop-shadow-2xl mb-6 transition-all" />
                )}
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-2 w-full max-w-md no-scrollbar justify-center">
                    {previewImagesList.map((img: string, idx: number) => (
                      <button 
                        ...
                        className={`shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${activePreviewImage === idx ? 'border-primary scale-110' : 'border-white/10 hover:border-white/30 opacity-60 hover:opacity-100'}`}
                      >
              ...
              <div className="p-10 space-y-8 md:w-1/2 max-h-[85vh] overflow-y-auto no-scrollbar flex flex-col">
                <div>
                  <span className="bg-primary/20 text-primary text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-primary/20">
                    {selectedProductForPreview.categoria}
                  </span>
                  <h2 className="text-3xl font-black text-white mt-4 tracking-tighter uppercase leading-none">
                    {selectedProductForPreview.nombre}
                  </h2>
                  <p className="text-gray-500 text-sm mt-4 leading-relaxed italic">
                    {selectedProductForPreview.descripcion || "Sin descripción técnica configurada."}
                  </p>
                </div>
                          ...
                          <div key={key}>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {Array.isArray(vals) ? vals.map(v => (
                                <span key={v} className="bg-white/5 text-gray-400 text-[10px] px-2 py-1 rounded-md border border-white/5">{v}</span>
                              )) : (
                                <span className="bg-white/5 text-gray-400 text-[10px] px-2 py-1 rounded-md border border-white/5">{vals}</span>
                              )}
                            </div>
                          </div>
                        ))}
                ...
                <div className="flex gap-4 pt-6 border-t border-white/5 mt-auto">
                  <button onClick={() => handleEditFromPreview(selectedProductForPreview)} className="flex-1 bg-white/5 hover:bg-primary hover:text-white py-4 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all border border-white/5 hover:border-primary">
                    <Edit size={16}/> Editar 
                  </button>
                  <button onClick={() => { 
                    const id = selectedProductForPreview.id; 
                    setSelectedProductForPreview(null); 
                    triggerDelete(id, "Producto", async () => await deleteDoc(doc(db,"productos",id)));
                  }} className="px-6 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all flex items-center justify-center">
                    <Trash2 size={20}/>
                  </button>
                </div>
