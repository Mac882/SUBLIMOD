      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 grid items-start gap-7 lg:grid-cols-[1.02fr_0.98fr]">
          <section className="rounded-[2rem] border border-white/80 bg-white/90 p-3 shadow-[0_24px_60px_rgba(42,67,125,0.12)] backdrop-blur">
            ...
            {images.length > 1 && <div className="mt-3 grid grid-cols-5 gap-2">{images.map((image, index) => <button type="button" key={`${image}-${index}`} onClick={() => setActiveImage(index)} className={`overflow-hidden rounded-xl border-2 bg-white transition ${activeImage === index ? "border-[#3158ee]" : "border-transparent"}`} aria-label={`Seleccionar imagen ${index + 1}`} aria-current={activeImage === index ? "true" : undefined}><img src={image} alt="" className="aspect-square w-full object-contain p-1" /></button>)}</div>}
            <p className="mt-2 text-center text-[10px] font-semibold text-[#8a9ab4]">Selecciona una miniatura para cambiar la imagen principal · toca la imagen principal para abrir la galería</p>
          </section>
