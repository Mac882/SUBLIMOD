...
const ProductDetailModal = ({ product, onClose }: ProductDetailModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-secondary/35 p-2 backdrop-blur-md sm:p-3 lg:p-6">
      <div className="relative flex h-dvh w-full max-w-5xl flex-col overflow-hidden rounded-none border border-white bg-[#F8FAFA] shadow-2xl animate-in zoom-in duration-300 sm:h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-1.5rem)] sm:rounded-[2rem] lg:h-[min(900px,calc(100dvh-3rem))] lg:max-h-[calc(100dvh-3rem)] lg:rounded-[2.5rem]">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:grid-cols-2 lg:overflow-hidden">
          ...
          <div className="min-h-0 space-y-8 overflow-y-auto overscroll-contain bg-[#F8FAFA] p-5 sm:p-7 lg:space-y-10 lg:p-10 xl:p-12">
            ...
            <div className="space-y-8 rounded-[2rem] border border-primary/10 bg-white p-5 sm:p-8"><div className="flex flex-col items-center justify-between gap-6 md:flex-row"><div className="w-full space-y-3 md:w-auto"><label className="block text-[10px] font-black uppercase text-gray-500">Cantidad</label><div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-[#F8FAFA] p-1"><button type="button" onClick={() => setQuantity((current) => Math.max(1, current - 1))} className="p-3"><Minus size={20}/></button><span className="w-12 text-center text-xl font-black text-primary">{quantity}</span><button type="button" onClick={() => setQuantity((current) => current + 1)} className="p-3"><Plus size={20}/></button></div></div><div className="text-center md:text-right"><label className="block text-[10px] font-black uppercase text-gray-500">Total Estimado</label><div className="text-4xl font-black text-accent">C$ {totalPrice}</div><span className="text-[10px] font-bold uppercase text-gray-500">C$ {variantPrice} por unidad</span></div></div><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><button type="button" onClick={handleDirectOrder} className="flex items-center justify-center gap-3 rounded-2xl bg-primary py-5 text-sm font-black uppercase tracking-widest text-white"><MessageCircle size={20}/> Personalizar y Pedir</button><button type="button" onClick={handleAddToQuote} className={`flex items-center justify-center gap-3 rounded-2xl border-2 py-5 text-sm font-black uppercase ${addedToQuote ? "border-green-500 bg-green-500/10 text-green-500" : "border-gray-200 bg-white text-secondary"}`}>{addedToQuote ? <><Check size={20}/> ¡Añadido!</> : <><ShoppingCart size={20}/> Añadir a Cotización</>}</button></div></div>
