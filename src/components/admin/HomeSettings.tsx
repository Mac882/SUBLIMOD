...
const DEFAULTS: HomeConfig = {
  ...
  homeHeroDescription: "Productos sublimables de calidad para convertir tus ideas en piezas únicas, con color, detalle y ese toque que hace la diferencia.",
  homeHeroImageUrl: "",
  homeHeroImagePath: "",
  homeTechnologyTitle: "Laptops seleccionadas para trabajar, crear y avanzar",
  homeTechnologyDescription: "Una segunda línea de productos pensada para acompañarte en el trabajo, el estudio y tus proyectos.",
};
...
export default function HomeSettings() {
  ...
  useEffect(() => {
    return onSnapshot(doc(db, "configuracion", "general"), (snapshot) => {
      ...
      setConfig((current) => ({
        ...current,
        homeHeroTitle: textValue(data.homeHeroTitle, DEFAULTS.homeHeroTitle),
        homeHeroDescription: textValue(data.homeHeroDescription, DEFAULTS.homeHeroDescription),
        homeHeroImageUrl: textValue(data.homeHeroImageUrl, ""),
        homeHeroImagePath: textValue(data.homeHeroImagePath, ""),
        homeTechnologyTitle: textValue(data.homeTechnologyTitle, DEFAULTS.homeTechnologyTitle),
        homeTechnologyDescription: textValue(data.homeTechnologyDescription, DEFAULTS.homeTechnologyDescription),
      }));
    });
  }, []);
  ...
  const saveText = async () => {
    ...
    try {
      ...
      alert("No se pudo guardar el contenido del Home.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const uploadHeroImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (config.homeHeroImagePath) {
        try { await deleteObject(ref(storage, config.homeHeroImagePath)); }
        catch (error) { console.warn("No se pudo eliminar la imagen anterior del Home", error); }
      }
      ...
      const next = { ...config, homeHeroImageUrl: url, homeHeroImagePath: path };
  ...
  const removeHeroImage = async () => {
    ...
    try {
      if (config.homeHeroImagePath) {
        try { await deleteObject(ref(storage, config.homeHeroImagePath)); }
        catch (error) { console.warn("No se pudo eliminar el archivo del Storage", error); }
      }
      const next = { ...config, homeHeroImageUrl: "", homeHeroImagePath: "" };
      await setDoc(doc(db, "configuracion", "general"), next, { merge: true });
      setConfig(next);
    } catch (error) {
  ...
  return (
    <section className="space-y-6 border-t border-white/5 pt-6">
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-gray-500">Título, descripción e imagen. La imagen se gestiona únicamente con subir, cambiar o eliminar.</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {config.homeHeroImageUrl ? (
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-black/20">
            ...
            <button type="button" onClick={removeHeroImage} disabled={uploading} className="absolute right-3 top-3 inline-flex items-center gap-2 rounded-xl bg-red-500 px-3 py-2 text-[9px] font-black uppercase tracking-widest text-white shadow-lg disabled:opacity-50">
              <Trash2 size={14} /> Eliminar
            </button>
          </div>
        ) : (
          <div className="flex min-h-36 items-center justify-center rounded-3xl border border-dashed border-white/10 bg-black/20 p-6 text-center">
            <div><ImagePlus className="mx-auto text-primary" size={34} /><p className="mt-3 text-xs font-black uppercase tracking-widest text-gray-400">Sin imagen principal</p></div>
          </div>
        )}
